...
static tcp_seg inseg;


/* The TCP PCB lists. */
extern tcp_pcb_listen *tcp_listen_pcbs;  /* List of all TCP PCBs in LISTEN state. */
extern tcp_pcb *tcp_active_pcbs;  /* List of all TCP PCBs that are in a
				 state in which they accept or send
				 data. */
extern tcp_pcb *tcp_tw_pcbs;      /* List of all TCP PCBs in TIME-WAIT. */

extern tcp_pcb *tcp_tmp_pcb;

extern u32_t tcp_ticks;

static int tcp_process(tcp_pcb *pcb);
static void tcp_parseopt(tcp_pcb *pcb);
static void tcp_receive(tcp_pcb *pcb);
...
void tcp_input(pbuf *p, netif *inp)
{
  ...
  ++stats.tcp.recv;
	...
	iphdr = p->payload;
  	tcphdr = (tcp_hdr *)((u8_t *)p->payload + IPH_HL(iphdr) * 4/sizeof(u8_t));
  	...
  	if(ip_addr_isbroadcast(&(iphdr->dest), &(inp->netmask)) || ip_addr_ismulticast(&(iphdr->dest))) 
	{
  	...
  	/* Verify TCP checksum. */
  	if(inet_chksum_pseudo(p, (ip_addr *)&(iphdr->src), (ip_addr *)&(iphdr->dest), IP_PROTO_TCP, p->tot_len) != 0) 
	{
   		printf("tcp_input: packet discarded due to failing checksum 0x%04x\n", inet_chksum_pseudo(p, (ip_addr *)&(iphdr->src),(ip_addr *)&(iphdr->dest), IP_PROTO_TCP, p->tot_len));
    		...
    		if(pcb == NULL) 
		{
      			for(pcb = (tcp_pcb *)tcp_listen_pcbs; pcb != NULL; pcb = pcb->next) 
			{
				...
				{	  
					...
					{
	    					prev->next = pcb->next;
	    					pcb->next =  (tcp_pcb *)tcp_listen_pcbs;
	    					tcp_listen_pcbs = (tcp_pcb_listen *)pcb; 
	  				}
	  				break;
				}
				prev = pcb;
...
static int tcp_process(tcp_pcb *pcb)
{
  	...
  	iphdr = (ip_hdr *)((u8_t *)inseg.tcphdr - IP_HLEN/sizeof(u8_t));
  	tcphdr = inseg.tcphdr;
  	...
  	if(flags & TCP_RST) 
	{
		...
		{
      			printf("tcp_process: unacceptable reset seqno %lu rcv_nxt %lu\n", seqno, pcb->rcv_nxt);
      			printf("tcp_process: unacceptable reset seqno %lu rcv_nxt %lu\n", seqno, pcb->rcv_nxt);
    		}
  	...
  	switch(pcb->state) 
	{
  		...
  		case LISTEN:
    			...
    			if(flags & TCP_ACK) 
			{
      				...
      				printf("tcp_process: ACK in LISTEN, sending reset\n");
      				tcp_rst(ackno + 1, seqno + TCP_TCPLEN(&inseg), &(iphdr->dest), &(iphdr->src),
	      tcphdr->dest, tcphdr->src);
    			} 
			else if(flags & TCP_SYN) 
			{
      				printf("TCP connection request %d -> %d.\n", inseg.tcphdr->src, inseg.tcphdr->dest);
      				npcb = tcp_new();
        ...
        ++stats.tcp.memerr;
    			...
    			break;
  		case SYN_SENT:
    			printf("SYN-SENT: ackno %lu pcb->snd_nxt %lu unacked %lu\n", ackno,
	   pcb->snd_nxt, ntohl(pcb->unacked->tcphdr->seqno));
    			if(flags & TCP_ACK && flags & TCP_SYN && ackno == ntohl(pcb->unacked->tcphdr->seqno) + 1) 
			{
      				...
      				pcb->cwnd = pcb->mss;
      				--pcb->snd_queuelen;
      				printf("tcp_process: SYN-SENT --queuelen %d\n", pcb->snd_queuelen);
      				rseg = pcb->unacked;
...
static void tcp_receive(tcp_pcb *pcb)
{
  	...
  	if(TCPH_FLAGS(inseg.tcphdr) & TCP_ACK) 
	{
       			...
       			(pcb->snd_wl2 == ackno && inseg.tcphdr->wnd > pcb->snd_wnd)) 
		{
      			...
      			pcb->snd_wl1 = seqno;
      			pcb->snd_wl2 = ackno;
      			printf("tcp_receive: window update %lu\n", pcb->snd_wnd);
#if TCP_WND_DEBUG
    } else {
      if(pcb->snd_wnd != inseg.tcphdr->wnd) {
        printf("tcp_receive: no window update lastack %lu snd_max %lu ackno %lu wl1 %lu seqno %lu wl2 %lu\n",
    		...
    		if(pcb->lastack == ackno) 
		{
      			...
      			if(pcb->dupacks >= 3 && pcb->unacked != NULL) 
			{
        			if(!(pcb->flags & TF_INFR)) 
				{
          				...
          				printf("tcp_receive: dupacks %d (%lu), fast retransmit %lu\n",
                                		pcb->dupacks, pcb->lastack, ntohl(pcb->unacked->tcphdr->seqno));
          				tcp_rexmit_seg(pcb, pcb->unacked);
          				/* Set ssthresh to max (FlightSize / 2, 2*SMSS) */
          				pcb->ssthresh = UMAX((pcb->snd_max - pcb->lastack) / 2, 2 * pcb->mss);
 ...
 the value overflows. */
		...
		else if(TCP_SEQ_LT(pcb->lastack, ackno) && TCP_SEQ_LEQ(ackno, pcb->snd_max)) 
		{
      			...
      			if(pcb->state >= ESTABLISHED) 
			{
        			if(pcb->cwnd < pcb->ssthresh) 
				{
	  				if(pcb->cwnd + pcb->mss > pcb->cwnd) 
					{
          				...
          				printf("tcp_receive: slow start cwnd %u\n", pcb->cwnd);
        			} 
				else 
				{
	  				if(pcb->cwnd + pcb->mss * pcb->mss / pcb->cwnd > pcb->cwnd) 
					{
          				...
          				printf("tcp_receive: congestion avoidance cwnd %u\n", pcb->cwnd);
        			}
      			}
      
			printf("tcp_receive: ACK for %lu, unacked->seqno %lu:%lu\n",
                               ackno,
                               pcb->unacked != NULL?
			...
			{
				printf("tcp_receive: removing %lu:%lu from pcb->unsent\n",
				...
				printf("tcp_receive: queuelen %d ... ", pcb->snd_queuelen);
				pcb->snd_queuelen -= pbuf_clen(next->p);
				tcp_seg_free(next);
				printf("%d (after freeing unsent)\n", pcb->snd_queuelen);
#ifdef LWIP_DEBUG
	  ...
	  ASSERT("tcp_receive: valid queue length", pcb->unacked != NULL ||
		 pcb->unsent != NULL);      
        			...
        			if(pcb->unsent != NULL) 
				{
      			...
      			while(pcb->unacked != NULL && TCP_SEQ_LEQ(ntohl(pcb->unacked->tcphdr->seqno) +
				TCP_TCPLEN(pcb->unacked), ackno)) 
			{
				
				printf("tcp_receive: removing %lu:%lu from pcb->unacked\n",
				...
				printf("%d (after freeing unacked)\n", pcb->snd_queuelen);
#ifdef LWIP_DEBUG
	if(pcb->snd_queuelen != 0) {
	  ASSERT("tcp_receive: valid queue length", pcb->unacked != NULL ||
		 pcb->unsent != NULL);      
    		...
    		printf("tcp_receive: pcb->rttest %d rtseq %lu ackno %lu\n", pcb->rttest, pcb->rtseq, ackno);
    		...
    		if(pcb->rttest && TCP_SEQ_LT(pcb->rtseq, ackno)) 
		{
      			...
      			printf("tcp_receive: experienced rtt %d ticks (%d msec).\n",m, m * TCP_SLOW_INTERVAL);
      			...
      			if(m < 0) 
			{
				m = -m;
      			...
      			printf("tcp_receive: RTO %d (%d miliseconds)\n", pcb->rto, pcb->rto * TCP_SLOW_INTERVAL);
  	...
  	if(TCP_TCPLEN(&inseg) > 0) 
	{
    		...
    		*/
    		...
    		if(TCP_SEQ_LT(seqno, pcb->rcv_nxt) && TCP_SEQ_LT(pcb->rcv_nxt, seqno + inseg.len)) 
		{
	 		...
	 		adjust the ->data pointer in the seg and the segment length.*/
	...
	if(pcb->ooseq != NULL &&
	  ...
	  pbuf_realloc(inseg.p, inseg.len);
				...
				{
	  				printf("tcp_receive: received FIN.");
	  				...
	  				for(next = pcb->ooseq; next != NULL; next = next->next) 
					{
	    					if(seqno == next->tcphdr->seqno) 
						{
	      						/* The sequence number of the incoming segment is the
                 					...
                 					->ooseq. We check the lengths to see which one to discard. */
	      						if(inseg.len > next->len) 
							{
								/* The incoming segment is larger than the old
                   						segment. We replace the old segment with the new one. */
						...
						else 
						{
	      						if(prev == NULL) 
							{
								...
								{
		  							/* The sequence number of the incoming segment is 										lower than the sequence number of the first segment on the
		     							queue. We put the incoming segment first on thequeue. */
							...
							else if(TCP_SEQ_LT(prev->tcphdr->seqno, seqno) && TCP_SEQ_LT(seqno, next->tcphdr->seqno)) 
							{
								/* The sequence number of the incoming segment is in
                   						...
                   						incoming segment and trim the previous segment, if needed. */
								if(TCP_SEQ_GT(seqno + inseg.len, next->tcphdr->seqno)) 
								{
		  							/* We need to trim the incoming segment. */
		  							inseg.len = next->tcphdr->seqno - seqno;
								...
								break;
	      						...
	      						/* If the "next" segment is the last segment on the
                 					ooseq queue, we add the incoming segment to the end of the list. */
