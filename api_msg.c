...
static err_t recv_tcp(void *arg, tcp_pcb *pcb, pbuf *p, err_t err)
{
  	struct netconn *conn;
...
/*-----------------------------------------------------------------------------------*/
static void recv_udp(void *arg, udp_pcb *pcb, pbuf *p, ip_addr *addr, u16_t port)
{
	struct netbuf *buf;
  	struct netconn *conn;
...
/*-----------------------------------------------------------------------------------*/
static err_t poll_tcp(void *arg, tcp_pcb *pcb)
{
  	struct netconn *conn;

  	conn = arg;
  	if(conn != NULL && (conn->state == NETCONN_WRITE || conn->state == NETCONN_CLOSE) &&
...
/*-----------------------------------------------------------------------------------*/
static err_t sent_tcp(void *arg, tcp_pcb *pcb, u16_t len)
{
  	struct netconn *conn;
...
/*-----------------------------------------------------------------------------------*/
static err_t accept_function(void *arg, tcp_pcb *newpcb, err_t err)
{
  	sys_mbox_t *mbox;
...
/*-----------------------------------------------------------------------------------*/
static err_t do_connected(void *arg, tcp_pcb *pcb, err_t err)
{
  	struct netconn *conn;
  	...
  	if(msg->conn->pcb.tcp != NULL) 
	{
    		switch(msg->conn->type) 
		{
    			...
    			case NETCONN_UDPNOCHKSUM:
      			/* FALLTHROUGH */
    			case NETCONN_UDP:
      				printf("api_msg: listen UDP: cannot listen for UDP.\n");
      				break;
    			case NETCONN_TCP:
  	...
  	if(msg->conn->pcb.tcp != NULL) 
	{
    		switch(msg->conn->type) 
		{
    			...
    			case NETCONN_UDPNOCHKSUM:
      			/* FALLTHROUGH */
    			case NETCONN_UDP:    
      				printf("api_msg: accept UDP: cannot accept for UDP.\n");
      				break;
    			case NETCONN_TCP:
      				break;
    		}
