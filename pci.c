...
u32_t pci_get_vendor_and_device(u16_t bus, u16_t slot, u16_t func);




u32_t pci_read_config(u16_t bus, u16_t slot, u16_t func, u16_t offset)
{
	u32_t address;
	...
	address = (u32_t)((lbus << 16) | (lslot << 11) | (lfunc << 8) | (offset & 0xfc) | ((u32_t)0x80000000));

	outl(0xCF8, address);
	return inl(0xCFC);
}

void pci_write_config_word(u16_t bus, u16_t slot, u16_t func, u16_t offset, u16_t value)
{
	u32_t address;
	u32_t lbus = (u32_t)bus;
	u32_t lslot = (u32_t)slot;
...
const char* pci_subclass_name(u8_t class, u8_t subclass, u8_t progif)
{
	int search = 0;
...
PCI_Device* pci_find(u16_t ven, u16_t dev)
{
	...
	{
		vendor_id = cur->vendordevice & 0xFFFF;
		device_id = (cur->vendordevice >> 16) & 0xFFFF;
		if (ven == vendor_id && dev == device_id)	return cur;
	}
	return NULL;
}


PCI_Device* pci_find_device(u8_t class, u8_t subClass)
{
	PCI_Device* cur = pci_devices;
	...
	for (; cur; cur = cur->next)
	{
		if (cur->deviceclass == class && cur->devicesubclass == subClass)	return cur;
	}
	return NULL;
}

void list_pci(u8_t showbars)
{
	PCI_Device* cur = pci_devices;
	for (; cur; cur = cur->next)
	{
//		printf("class = %d , subcalss = %d\n", cur->deviceclass, cur->devicesubclass);
		printf("%04x:%04x - on %d:%d.%d: %s\n",
				cur->vendordevice & 0xFFFF,
				(cur->vendordevice >> 16) & 0xFFFF,
				cur->bus, cur->slot, cur->func,
				pci_subclass_name(cur->deviceclass, cur->devicesubclass, cur->deviceif),cur->deviceclass, cur->devicesubclass, cur->deviceif);
		if (showbars)
		{
			int bar = 0;
...
void scan_pci_bus(int bus)
{
	...
	for (; slot < 63; slot++)
	{
		...
		while (func < 64)
		{
			u32_t id = pci_get_vendor_and_device(bus, slot, func);
			if ((id & 0xFFFF) != 0xFFFF)
			{
				u32_t flags = pci_read_config(bus, slot, func, PCI_OFS_FLAGS);
				u32_t class = pci_read_config(bus, slot, func, PCI_OFS_CLASS);
				u32_t headertype = (flags >> 16) & 0x7f;
				if ((flags >> 16) & 0x80)
					mf = 1;
				...
				if (headertype == 0x00)
				{
					u32_t irq = pci_read_config(bus, slot, func, PCI_OFS_IRQ);
					irq &= 0xFF;
					dev->irq = irq;
				}
				if (headertype == 0x01)		/* PCI/PCI bus */
				{
					u32_t secondary_bus = pci_read_config(bus, slot, func, PCI_OFS_SECONDARYBUS);
					u32_t sb = secondary_bus;
				...
				{
					u32_t b = pci_read_config(bus, slot, func, PCI_OFS_BARS + (bar * 4));
					if ((b & 1) == 0)
					{
...
PCI_DEVTABLE	PciDevTable [] =
{
	...
	{ 0x1002, 0x0000, "{4D36E968-E325-11CE-BFC1-08002BE10318}", "{4D36E968-E325-11CE-BFC1-08002BE10318}" } ,
	...
	{ 0x1002, 0x5A41, "0x5A41 ATI RADEON Xpress Series	0x1002", "0x5A41	ATI RADEON Xpress Series	0x1002" } ,
	...
	{ 0x1014, 0x0000, "Unknown", "IBM Netfinity Advanced System Management Processor" } ,
	{ 0x1014, 0x0000, "n/a", "n/a" } ,
	...
	{ 0x1057, 05600, "52-6116-2A", "Motorola FM 56 PCI Speakerphone Modem" } ,
	...
	{ 0x13F0, 0x13F0, "PCI\\VEN_13F0&DEV_0000&SUBSYS_82131043&REV_31\\4&150", "ST201 Fast Ethernet" } ,
	{ 0x13FD, 0x160E, "a", "a" } ,
	...
	{ 0x11D1, 0x0520, "tag4769", "Video card" } ,
...
void pci_detect_device(u8_t class, u8_t subClass)
{
	PCI_Device* cur = pci_devices;
...
//printf("--class = %d , subcalss = %d\n", class, subClass);
	...
	{
//printf("*class = %d , subcalss = %d\n", cur->deviceclass, cur->devicesubclass);
		if (cur->deviceclass == class && cur->devicesubclass == subClass)
		{
