...
void netbuf_copy_partial(struct netbuf *buf, void *dataptr, u16_t len, u16_t offset)
{
  	...
  	if(buf == NULL) 
	{
  	...
  	/* This implementation is bad. It should use bcopy instead. */
...
err_t netconn_delete(struct netconn *conn)
{
  	...
  	if(conn == NULL) 
	{
  	...
  	if((msg = malloc(sizeof(struct api_msg))) == NULL) 
	{
    		return ERR_MEM;
  	}
...
struct netbuf* netconn_recv(struct netconn *conn)
{
	...
	else 
	{
  	...
  	printf("netconn_recv: received %p (err %d)\n", buf, conn->err);
...
err_t netconn_send(struct netconn *conn, struct netbuf *buf)
{
  	...
  	if((msg = memp_mallocp(MEMP_API_MSG)) == NULL) 
	{
  	...
  	printf("netconn_send: sending %d bytes\n", buf->p->tot_len);
...
err_t netconn_write(struct netconn *conn, void *dataptr, u16_t size, u8_t copy)
{
  	...
  	while(conn->err == ERR_OK && size > 0) 
	{
		...
		else 
		{
    		...
    		printf("netconn_write: writing %d bytes (%d)\n", len, copy);
