...
int lwip_recvfrom(int s, void *mem, int len, unsigned int flags, struct sockaddr *from, int *fromlen)
{
	...
	else 
	{
  	...
  	/* If this is a TCP socket, check if there is data left in the buffer. If so, it should be saved in the sock structure for next
     		time around. */
...
/*-----------------------------------------------------------------------------------*/
int
lwip_sendto(int s, void *data, int size, unsigned int flags, struct sockaddr *to, int tolen)
{
  	struct lwip_socket *sock;
...
int lwip_socket(int domain, int type, int protocol)
{
  	...
  	switch(type) 
	{
  	...
  	if(conn == NULL) 
	{
    		printf("socket: could not create netconn.\n");
    		/* errno = ENOBUFS; */
    		return -1;
  	}
